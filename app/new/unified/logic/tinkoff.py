...
from logic.db import BASE_DIR
import json
from pathlib import Path

BASE_URL = CONFIG["tinkoff"]["base_url"]

def get_token_for_user(nick: str = "Admin") -> str:
    # 1. Попытка прочитать токен конкретного пользователя из настроек Seller бота
    root_dir = BASE_DIR.parent.parent
    user_config_path = root_dir / "data" / "users" / nick / "apps" / "Seller" / "config.server.json"
    if user_config_path.exists():
        try:
            with open(user_config_path, "r", encoding="utf-8") as f:
                data = json.load(f)
                tok = data.get("settings", {}).get("TOKEN", "").strip()
                if tok:
                    return tok
        except Exception as e:
            log.warning(f"Error reading Seller token for {nick}: {e}")
            
    # 2. Фолбек на глобальный токен
    return CONFIG["tinkoff"]["token"]

async def t_post(path: str, body: dict, user: str = "Admin") -> dict:
